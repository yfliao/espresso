...
if [ ${stage} -le 4 ]; then
  ...
  asr_prep_json.py --feat-files $train_feat --numerator-fst-files $train_fst --text-files $train_text --utt2num-frames-files $train_utt2num_frames --output data/chain_e2e/train.json
  asr_prep_json.py --feat-files $valid_feat --numerator-fst-files $valid_fst --text-files $valid_text --utt2num-frames-files $valid_utt2num_frames --output data/chain_e2e/valid.json
...
if [ ${stage} -le 5 ]; then
  ...
  CUDA_VISIBLE_DEVICES=$free_gpu speech_train.py data/chain_e2e --task speech_recognition_hybrid --seed 1 --user-dir espresso \
    ...
    --den-leaky-hmm-coefficient 1e-5 --num-leaky-hmm-coefficient 1e-20 \
